"""
Train Scalp Model - обучение модели для 5m/15m таймфреймов
...
def load_and_prepare_data(data_path: str) -> Tuple[np.ndarray, np.ndarray, list]:
    """
    Загрузка и подготовка данных для scalp модели
    
    Args:
        data_path: путь к CSV файлу с OHLCV данными
...
def train_scalp_model(
    ...
    """
    Обучение Scalp модели с TimeSeriesSplit валидацией
    
    Args:
        X: features
        y: targets
        feature_names: названия фичей
        n_splits: количество фолдов для валидации
        num_boost_round: максимум итераций
    ...
    """
    print("\n" + "="*50)
    print("Training Scalp Model (LightGBM)")
    print("="*50)
...
def save_model(model: lgb.Booster, feature_names: list, metrics: Dict, model_path: str = None):
    """Сохранение модели"""
    model_path = model_path or MODEL_PATHS['scalp']
    ...
    # Save with pickle (includes feature names)
...
def predict(model: lgb.Booster, X: np.ndarray) -> Dict:
    ...
    # proba shape: (n_samples, 3) - [P_down, P_flat, P_up]
    return {
        'P_down': proba[:, 0],
        'P_flat': proba[:, 1],
        'P_up': proba[:, 2],
        'expected_return': proba[:, 2] - proba[:, 0]  # Simplified expected return
    }
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Train Scalp Model')
    ...
    # Train
    model, metrics = train_scalp_model(
        X, y, feature_names,
